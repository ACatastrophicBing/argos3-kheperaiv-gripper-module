...
/****************************************/
/*Edited to have the correct order and gripper*/
/****************************************/
...
void CBuzzControllerKheperaIV::Init(TConfigurationNode& t_node) {
   try {
      ...
      try {
         // printf("We have initialized the differential steering");
...
void CBuzzControllerKheperaIV::SetWheels(Real f_left_speed,
                                         Real f_right_speed) {
   // printf("Trying to change the wheel speeds\n");
...
// void CBuzzControllerKheperaIV::GetWheels() {
//    printf("Trying to get the wheel speeds\n");
//    printf(m_pcWheels->GetWheelVelocity(1));
//    printf("\n");
// }
...
void CBuzzControllerKheperaIV::GripperUnlock() {
   m_pcGripper->Unlock();
}
...
void CBuzzControllerKheperaIV::TurretEnable() {
   m_pcTurret->SetMode(CCI_KheperaIVTurretActuator::MODE_POSITION_CONTROL);
}

/****************************************/
/****************************************/

void CBuzzControllerKheperaIV::TurretDisable() {
   m_pcTurret->SetMode(CCI_KheperaIVTurretActuator::MODE_OFF);
}

/****************************************/
/****************************************/

void CBuzzControllerKheperaIV::TurretSet(Real f_rotation) {
   m_pcTurret->SetRotation(CRadians(f_rotation));
}
...
buzzvm_state CBuzzControllerKheperaIV::RegisterFunctions() {
   ...
   if(m_pcWheels) {
      ...
      buzzvm_gstore(m_tBuzzVM);
      /* BuzzGetWheels - trying to get wheel speeds but differential steering has no get*/
      // buzzvm_pushs(m_tBuzzVM, buzzvm_string_register(m_tBuzzVM, "get_wheels", 1));
      // buzzvm_pushcc(m_tBuzzVM, buzzvm_function_register(m_tBuzzVM, BuzzGetWheels));
      // buzzvm_gstore(m_tBuzzVM);
